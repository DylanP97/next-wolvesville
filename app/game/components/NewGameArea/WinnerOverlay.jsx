  ...
  return (
    <div
      ...
      <div className="winner-message flex flex-col justify-center align-center">
        <Image src={winningTeam.image} alt="winner" height={80} width={80} className="mb-2 w-full max-h-16 object-contain" />
        <p className="mb-4">The {winningTeam.name} won!</p>
        <div className="flex flex-row justify-center mt-4">
          {
            winningTeam.aliveList.map((ply) => {
              return (
                <div key={ply.name + 'key-win-div'} className="flex flex-col items-center w-44">
                  <div className="flex relative">
                    ...
                    <div className="p-2 bg-blue-500 rounded-full ">
                      <AvatarUI
                        avatar={ply.avatar} heightAndWidth={80}
                      />
                    </div>
                  </div>
                  <p className="m-2 text-gray-200 text-sm text-clip italic">{ply.name} as {ply.role.name}</p>
                </div>
              )
            })
          }
