// Separate logic for werewolf night voting (choosing who to kill)


function performWolfVote(playersList, cpu, socket, gameId) {
    ...
    if (wolfVotingState.leadingTarget && wolfVotingState.leadingVotes >= 2) {
        ...
        if (Math.random() < 0.75) {
            emitWolfVote(socket, cpu, wolfVotingState.leadingTarget, voteWeight, gameId);
            return;
        }
    }
    ...
    if (revealedDoctor && Math.random() < 0.65) {
        emitWolfVote(socket, cpu, revealedDoctor, voteWeight, gameId);
        return;
    }
...
function getWolfVotingState(killableTargets) {
...
function identifySuspiciousVillagers(unrevealedTargets, playersList) {
...
function emitWolfVote(socket, cpu, target, voteWeight, gameId) {
    socket.emit(
        "addWolfVote",
        {
            playerId: cpu.id,
...
//88
export default performWolfVote;